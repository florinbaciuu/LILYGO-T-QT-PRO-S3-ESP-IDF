 ...
 */
/* Initialize from: https://github.com/espressif/esp-idf/blob/v5.2.2/examples/system/console/advanced/components/cmd_nvs/cmd_nvs.c */
...
static char current_namespace[16] = "nvs_namespace";
...
static esp_err_t store_blob(nvs_handle_t nvs, const char *key, const char *str_values)
{
    ...
    if (blob == NULL) {
        ESP_LOGE(TAG, "@MALLOC_FAIL,%s,%d", __func__, __LINE__);
...
static void print_blob(const char *blob, size_t len)
{
    ...
    for (int i = 0; i < len; i++) {
        sprintf(buf + i * 2, "%02x", blob[i]);
    }
    ESP_LOGI(TAG, "nvs value: blob: %s", buf);
    free(buf);
}
...
static esp_err_t set_value_in_nvs(const char *key, const char *str_type, const char *str_value)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to open nvs %s", current_namespace);
        return err;
    }
    ESP_LOGI(TAG, "set_nvs key: %s, type: %s, val: %s", key, str_type, str_value);
    ...
    if (err == ESP_OK) {
        err = nvs_commit(nvs);
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "nvs_set OK.");
        }
    } else {
        ESP_LOGE(TAG, "nvs_set Failed! err: %s", esp_err_to_name(err));
    }
...
static esp_err_t get_value_from_nvs(const char *key, const char *str_type)
{
    ...
    ESP_LOGI(TAG, "getting nvs key: %s", key);

    if (type == NVS_TYPE_I8) {
        int8_t value;
        err = nvs_get_i8(nvs, key, &value);
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "nvs value: %d", value);
        }
    } else if (type == NVS_TYPE_U8) {
        uint8_t value;
        err = nvs_get_u8(nvs, key, &value);
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "nvs value: %u", value);
        }
    } else if (type == NVS_TYPE_I16) {
        int16_t value;
        ...
        if ((err = nvs_get_i64(nvs, key, &value)) == ESP_OK) {
            ESP_LOGI(TAG, "nvs value: %lld", value);
        }
    } else if (type == NVS_TYPE_U64) {
        uint64_t value;
        if ( (err = nvs_get_u64(nvs, key, &value)) == ESP_OK) {
            ESP_LOGI(TAG, "nvs value: %llu", value);
        }
    } else if (type == NVS_TYPE_STR) {
        size_t len;
...
#else
    result = nvs_entry_find(part, NULL, type, &it);
#endif
    if (result == ESP_ERR_NVS_NOT_FOUND || !it) {
...
#endif
        ESP_LOGI(TAG, "nvs value: namespace '%s', key '%s', type '%s' \n",
                 info.namespace_name, info.key, type_to_str(info.type));
    } while (result == ESP_OK && it);
...
static int set_value(int argc, char **argv)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "nvs_set Failed. %s", esp_err_to_name(err));
        return 1;
    }
...
static int get_value(int argc, char **argv)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "nvs_get Failed. %s", esp_err_to_name(err));
        return 1;
    }
...
static int erase_value(int argc, char **argv)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "nvs_earse Failed. %s", esp_err_to_name(err));
        return 1;
    }
    ESP_LOGI(TAG, "nvs erase value OK. key: %s", key);
    return 0;
}
...
static int erase_namespace(int argc, char **argv)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "nvs_earse_namespace Failed. %s", esp_err_to_name(err));
        return 1;
    }
    ESP_LOGI(TAG, "nvs erase namespace OK. namespace: %s", name);
    return 0;
}
